...
template<typename T>T Radian (T Degree)
{
...
int main()
{
    ...
    LOOP(caseno, 1, test){
        double r, angleB;
        scanf("%lf %lf", &r, &angleB);
        angleB = Radian(angleB);
        double halfB = angleB / 2.0;
        double HO = r * tan(halfB);
        double angleN = Radian(90.0);
        double angleA = Radian(180.0) - (halfB + angleN);
        double NB = ((2.0 * r)/sin(angleN)) * sin(angleA);
        double OB = sqrt((HO * HO) + (r * r));

        // NO/NF = IO/DF
        // (NB - OB) / ( NB - BF ) = IO / (DE - EF )

        double angleC = Radian(90.0);
        double angleA2 = Radian(180.0) - (angleC + angleB);
        double BC = ((2.0 * r)/sin(angleC)) * sin(angleA2);
        double halfBC = BC / 2.0;


        double angleE = Radian(90.0);
        double angleD = Radian(180.0) - (angleB + angleE);
        double angleF = Radian(180.0) - (halfB + angleE);
        double DE = (halfBC / sin(angleE)) * sin(angleB);
        double BE = (halfBC / sin(angleE)) * sin(angleD);
        double EF = ((BE / sin(angleF))) * sin(halfB);
        double BF = ((BE / sin(angleF))) * sin(angleE);
